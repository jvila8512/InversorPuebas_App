...
@dataclass
class ModbusStatus:
    """Estado del inversor leído via Modbus RTU."""
    # Raw registers
    raw_registers: list = field(default_factory=list)
    
    # Modos
    working_mode: WorkingMode = WorkingMode.STANDBY
    charge_mode: ChargeMode = ChargeMode.NONE
    
    # Batería
    battery_voltage: float = 0.0      # V
    battery_charge_a: float = 0.0     # A (cargando)
    battery_discharge_a: float = 0.0  # A (descargando)
    battery_charge_w: float = 0.0     # W (cargando)
    battery_discharge_w: float = 0.0  # W (descargando)
    ...
    grid_frequency: float = 0.0      # Hz (no disponible en Modbus)
...
class ModbusSettings:
    """Configuración del inversor leída via Modbus RTU."""
    raw_registers: list = field(default_factory=list)
    
    # Voltajes de batería
    battery_cutoff_voltage: float = 0.0    # V (corte por descarga)
    battery_cv_voltage: float = 0.0        # V (absorción/CV)
    battery_float_voltage: float = 0.0     # V (flotación)
    battery_back_to_grid_voltage: float = 0.0  # V (regreso a red)
    battery_back_to_battery_voltage: float = 0.0  # V (regreso a batería)
    
    # Prioridades
    output_priority: OutputPriority = OutputPriority.SOLAR_FIRST
    charge_priority: ChargePriority = ChargePriority.SOLAR_AND_UTILITY
    
    # Corrientes
    max_charge_current: int = 0           # A
    max_ac_charge_current: int = 0        # A
...
class ModbusConnection:
    ...
    def read_status(self) -> Optional[ModbusStatus]:
        ...
        Lee los registros 0x1101 - 0x112A (42 registros).
        Implementación basada en FelicityInverter.cs línea 56-92.
        """
        ...
        # Parsear registros según FelicityInverter.cs:
        # Offset 0 (0x1101): Working mode
        status.working_mode = WorkingMode.from_int(regs[0])

        # Offset 1 (0x1102): Charge mode
        status.charge_mode = ChargeMode.from_int(regs[1])

        # Offset 7 (0x1108): Battery voltage / 100
        status.battery_voltage = round(regs[7] / 100.0, 1)

        # Offset 8 (0x1109): Battery current (signed)
        current = self._signed_value(regs[8])
        if current < 0:
            status.battery_discharge_a = abs(current)
        else:
            status.battery_charge_a = current

        # Offset 9 (0x110A): Battery power (signed)
        power = self._signed_value(regs[9])
        if power < 0:
            status.battery_discharge_w = abs(power)
        else:
            status.battery_charge_w = power

        # Offset 16 (0x1111): AC Output voltage / 10
        status.output_voltage = round(regs[16] / 10.0, 1)

        # Offset 22 (0x1117): Grid voltage / 10
        status.grid_voltage = round(regs[22] / 10.0, 1)

        # Offset 29 (0x111E): Load watts (raw)
        status.load_watts = regs[29]

        # Offset 31 (0x1120): Load percent (raw)
        status.load_percent = regs[31]

        # Offset 37 (0x1126): PV voltage / 10
        status.pv_voltage = round(regs[37] / 10.0, 1)

        # Offset 41 (0x112A): PV power (signed)
        status.pv_power = self._signed_value(regs[41])
    ...
    def read_settings(self) -> Optional[ModbusSettings]:
        ...
        Implementación basada en FelicityInverter.cs línea 99-117.
        """
        ...
        # Parsear según FelicityInverter.cs:
